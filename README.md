...
```bash
EventBridge$ sam build --use-container
```
...
```bash
EventBridge$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
EventBridge$ sam logs -n HelloWorldFunction --stack-name EventBridge --tail
```
...
```bash
EventBridge$ pip install pytest pytest-mock --user
EventBridge$ python -m pytest tests/ -v
```
